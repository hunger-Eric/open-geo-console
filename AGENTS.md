...
- `npm run public-search:probe -- --adapter <mimo|anysearch> --locale zh-CN --region CN` reads `.data/workstation-docker/staging.env`, the merged runtime environment used by the staging Docker Workers. Use `mimo` for `OGC_PROVIDER_PROFILE=mimo_native` and `anysearch` for `OGC_PROVIDER_PROFILE=external_search_synthesis`; do not point it back at source env files with empty Sensitive-value placeholders or diagnose a provider as missing from those placeholders alone.
...
- Customer report delivery is HTML-only. Prospective `free_direct` V3 and V4 activation must not require Chromium PDF generation, hashes, page-count checks, or PDF storage. Historical V1/V2 and pre-change V3 PDF records remain private and readable, with no customer PDF route, action, or email claim.
