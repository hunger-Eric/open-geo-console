...
- [ ] Restore Docker Desktop's Linux engine from an administrator session or workstation restart, then rebuild/recreate only the two staging Worker services from the current revision. Preview Web and the active report are live; production must remain untouched.
