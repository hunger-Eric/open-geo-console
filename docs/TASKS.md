...
- [x] Make V2 public-source safe retrieval abort the per-request dispatcher at `OGC_JOB_HARD_DEADLINE_MS` rather than waiting for graceful close; focused retriever and PostgreSQL recovery regressions pass. Before any new order, rebuild both controlled staging lanes from the reviewed revision and prove this path in a fresh paid drill.
