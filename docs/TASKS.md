...
- [x] Complete the V2 failure-path Sandbox settlement and delivery drill; payment, refund, and redirected test-email outcomes were delivered through Protected Preview.
...
- [x] Complete the provider refund and three redirected-email deliveries through Protected Preview.
...
- [x] Create one new Chinese free report and one CNY 199 Sandbox order after pre-order gates; prove exactly one signed-Webhook credit/deep job while preserving historical records.
...
- [ ] Resolve the outstanding Sandbox cash refund through a sanctioned provider reconciliation path; internal credit and email outcomes are already recorded, but cash refund is not complete.
...
- [ ] Resolve the outstanding Sandbox cash refund through a sanctioned provider path; internal credit is refunded and `db:audit` passes, but cash refund is not complete.
- [x] Deliver the paid report through an audited zero-charge replacement lineage without reopening its failed job; preserve original commercial state and open authorized HTML.
- [x] Activate a V3 evidence-refresh artifact: three nonblank question cards, one limited answer, two unresolved conclusions, four snapshot refs, private readiness, and authorized Chrome acceptance.
...
- [x] Rebuild/recreate only the two staging Worker services from the current acceptance revision; both now run `open-geo-console:staging-aee3690`, image ID `sha256:76c603238eb8602f12b82ef2f869aba90a4332606fc722ba1ea940e39a22984a`.
- [x] Reconcile the production free/deep Workers with schema v25 after explicit operator authorization: deploy `open-geo-console:prod-v25-11befe9` from revision `11befe9be1c2`, verify both readiness logs and zero restarts, pass `db:audit`, and preserve the production commerce container unchanged.
- [x] Complete a fully answered paid V3 acceptance with three answered cards, same-response sources, browser acceptance, private readiness, delivered email, and clean commercial audit.
- [x] Implement the provider-independent `generative_search_v1` contract, MiMo native-web-search adapter, same-response citation extraction, three-answer checkpoint-before-audit orchestration, answer-based commercial settlement, generative-first HTML rendering, and secret-safe staging probe.
- [x] Implement prospective `source_selection_diagnosis_v1`: derive source contributions, observable selection factors, target gaps and actions as a nonblocking post-audit checkpoint sidecar; persist it in V3; replace Section 03 counters with source profiles; preserve historical V3 fallback; and require all diagnosis prose in canonical HTML readiness. Full tests, lint and build pass.
- [x] Complete one protected-browser paid V3 acceptance after a real CNY 199 Sandbox checkout and signed Webhook. The immutable failed original job was not reopened; the audited non-billable replacement completed and the canonical customer HTML was accepted in Chrome.
- [x] Deploy and accept prospective V3 source-selection diagnosis with three answered cards, source profiles, desktop/mobile QA, private readiness, and independent delivery/refund truth.
...
- [ ] Complete anonymous-404 visual acceptance. Desktop/mobile canonical QA for the active V3 source-diagnosis report is complete; the in-app browser runtime remains unavailable on this workstation, so authenticated live-route visual evidence is still separate.

- [ ] Complete protected-staging acceptance for the grounded three-question presentation in `docs/superpowers/plans/2026-07-14-combined-geo-report-question-answer-presentation.md`. The combined-only answer contract, compact renderer, customer HTML/internal-PDF readiness, schema-v19 refresh lineage and guarded operator command are locally implemented and verified; deployment, artifact activation, Chromium screenshots and runtime evidence remain. This remains pending because the current V4 paid run is limited and all 20 requirements are still `implemented`, not `verified`.
...
- [x] Complete the real protected-staging correction: fresh snapshots, source-evidence rows, screenshots, internal readiness, atomic activation, correction email, no new billing/refund effects, and Chromium acceptance.
...
- [x] Accept one real V2 paid report end to end: paid/completed/delivered, full completion, settled credit, persisted public evidence, protected HTML/PDF readiness, and delivered transactional emails.
- [x] Recover and accept the paid Chinese V2 limited path: reuse persisted public-source evidence, finish `completed_limited`, complete the Sandbox refund, deliver all emails, and allow protected HTML access without exposing a customer PDF.
