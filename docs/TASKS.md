...
- [ ] Diagnose and fix the repeatable `public_source_snapshot_snapshot_materialization` failure observed before answer-card synthesis in terminal job `9f3221a2-1a3b-47c8-9c3e-eda2b8be52dd`; add deterministic coverage and rerun every pre-order gate before any replacement paid acceptance.
...
- [ ] Complete a fully answered/deliverable V3 acceptance on a target with independently retrievable direct public evidence. Order `98974ea3-369e-43bc-b84b-602d96382b02` proved exactly-one signed-Webhook admission but is now terminal failed and must not be reopened; no replacement order is authorized until the snapshot fix and all pre-order gates pass.
- [ ] Complete real Chrome desktop/mobile/authorized-report/404 acceptance after a V3 revision activates. The latest live run produced truthful 1440x1024 and 390x844 failure-state screenshots only; it did not produce an authorized customer report.
