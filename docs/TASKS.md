...
- [x] Make V2 public-source safe retrieval abort the per-request dispatcher at `OGC_JOB_HARD_DEADLINE_MS` rather than waiting for graceful close; focused retriever and PostgreSQL recovery regressions pass. Both controlled staging lanes were rebuilt from reviewed revision `c63dfc5` and reached ready state on 2026-07-14. Prove this path in a fresh paid drill before opening customer admission.
...
- [ ] Accept one real V2 paid report end to end: source collection/retrieval, immutable evidence, authorized HTML/PDF and anonymous `404`, transactional delivery/reply, and settlement. Fresh order `b1b9b382-962a-48f5-acf0-ba2f9b377c9e` passed payment and current-Worker claim but failed in public-source safe retrieval; it reached `paid/failed/refunded/delivered` with all three email Webhooks processed. This is failure evidence, not V2 acceptance.
