...
- [ ] Fix the live V3 artifact failure: Q1 generated `verified` confidence with only one independent registrable domain, then recovery returned a non-three-entry answer payload. Add red/green coverage and a specific bounded error classification before another paid attempt.
...
- [ ] Complete a fully answered/deliverable V3 acceptance on a target with independently retrievable direct public evidence. The latest report/order/job/revision are terminal failed/non-active and cannot serve as the acceptance artifact.
