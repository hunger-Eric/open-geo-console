...
- Schema v20 supports provider-discovery snapshot ancestry, immutable relevant passages and provider claims, `combined_geo_report_v2` artifact/access/job scopes, and staging-only `evidence_refresh` lineage while preserving all V1 rows.
...
- Deploy schema v20 and the V2 Web/Worker only to protected staging, opt in prospective staging jobs, and complete `docs/operations/provider-discovery-v2-acceptance.md`. Production activation remains forbidden.
- Restore a responsive staging PostgreSQL connection and provide an isolated `OGC_TEST_DATABASE_ADMIN_URL`. Local deterministic verification passes, but the staging security suite and read-only `db:audit` timed out without results on 2026-07-14; the two new disposable PostgreSQL suites therefore remain conditionally skipped.
- Complete the remaining protected-staging acceptance with one new English report and a fully completed Chinese deep/combined report. The paid Chinese V2 limited/refund/courtesy-access path is accepted, but it does not substitute for a full-completion artifact-readiness acceptance. Verify source-original labels, HTML-only email/UI, former PDF endpoints returning application `404`, and populated private PDF readiness fields for only those new revisions.
