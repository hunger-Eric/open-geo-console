...
- Fix the V3 answer-card confidence boundary so a sentence cannot reach `verified` without two independent registrable domains, and classify this contract failure specifically rather than as `unexpected_internal_error`. Preserve exactly three ordered answer entries across artifact recovery.
...
- Complete a fully answered Chinese V3 and one new English report only after deterministic fixes and all pre-order provider gates pass again. The new terminal failed order/artifact must not be reopened through an unsanctioned path.
