...
- New-report language enforcement is prospective: shared prompt/validation contracts cover page analysis, synthesis, combined answers, final combined prose, and localized application-owned labels. Newly materialized combined revisions also project deterministic technical findings and machine-readable asset summaries through the persisted locale before the final gate; captured page titles, H1 values, and URLs stay source-original and are labeled as such. Existing revisions remain unchanged. Evidence quotes, URLs, code, stable technical identifiers, and independently resolved entity names follow explicit exceptions; customer/profile/question text cannot create an allowlist.
