...
- PaymentIntent/HPP code is deployed to protected staging at `https://open-geo-console-staging-itheheda.vercel.app` through Preview `https://open-geo-console-cnw2llif9-itheheda-6857s-projects.vercel.app`. Anonymous deployment protection remains `302`/`401`. Browser acceptance used a fixed `USD 29.00` Sandbox PaymentIntent: cancel returned to the exact report without claiming payment, the successful card returned through the success URL, the banner waited for PostgreSQL, and the verified Webhook alone produced `paid + queued` and the exactly-once deep entitlement/job. Empty/non-JSON checkout failures now render localized retry copy. Unpaid legacy Payment Links are order-verified, age-gated, deactivated, rechecked, and atomically replaced by an HPP PaymentIntent; paid legacy links are never replaced or charged again. A missing free AI artifact now states that the technical report remains available instead of incorrectly claiming the whole deployment lacks AI configuration.
- The legacy Sandbox Payment Link for report `da7bc22c-d47f-407c-a827-6319efaa4d50` is provider-paid but its order remains PostgreSQL `pending` with zero recorded payment events because the signed Webhook was never delivered. The UI now explains that confirmation is pending without claiming payment. Re-trigger the provider event from Airwallex Sandbox before expecting the deep job; provider retrieval is not allowed to grant entitlement.
