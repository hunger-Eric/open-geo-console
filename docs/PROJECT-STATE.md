...
The root-cause repair is deployed as protected Preview `dpl_4EQQpkeqyM1v9zuw7NnR5AhXWw6P` at `https://open-geo-console-3r9vntoku-itheheda-6857s-projects.vercel.app`; the fixed staging alias points to it. Staging free/deep Docker Workers were rebuilt from the same source image; schema remains v21. Local deterministic validation and staging `db:audit` pass. No new V3 report, customer artifact activation, browser acceptance or Sandbox payment was completed, so customer-deliverable V3 acceptance remains open. Production and every prior report remain untouched.
...
- Complete a fully answered Chinese V3 and one new English report. There is no legal free refresh lineage for the non-active refunded V3 artifact: existing staging refresh supports only an active V1/V2 source revision, and submitted/completed refunds cannot be reopened. The next full run therefore stops at a new Airwallex Sandbox HPP unless a separately audited replacement-fulfillment mechanism is designed.
