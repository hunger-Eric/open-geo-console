...
The paid-acceptance remediation is deployed as protected Preview `dpl_56sV5LHa7Gb9W95VEVCCbvUtAeuj` at `https://open-geo-console-63n3rf4hc-itheheda-6857s-projects.vercel.app`; the fixed staging alias and staging-only free/deep Workers match source revision `fa4cdb28dbc9f877a7ac2c124b66d5cc122e46c7`. Schema is v22. The full deterministic suite (1,048 passed, 41 skipped), lint, build and staging `db:audit` pass. The first pre-order MiMo probe stopped because `OGC_PUBLIC_SEARCH_MIMO_BASE_URL` is missing from the staging probe environment, so Airwallex/Resend probes, checkout, payment, report activation, delivery and browser acceptance were not attempted. No new commercial or report IDs exist. Production, failed order `d738b38f-63cb-4886-bdda-c8f745bf5b81`, and every prior report remain untouched.
...
- Restore `OGC_PUBLIC_SEARCH_MIMO_BASE_URL` in the protected-staging probe environment, rerun the MiMo, Airwallex, Resend, deployment-alignment and commerce gates, and create a new Airwallex Sandbox order only if every gate passes. The prior failed/refunded orders and non-active artifacts must remain immutable.
