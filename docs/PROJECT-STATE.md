...
- Production is deployed at `https://geo.itheheda.online` through Cloudflare. Turnstile server verification is required for that hostname, Bot Fight Mode is enabled, AI-bot blocking remains off, and `/api/scan` has a Cloudflare 5 requests / 10 seconds / IP burst rule with a 10-second block. The production database marker fingerprint is `3b271c4714ee8b65`; schema-version inspection dropped from the one-time migration pass to about 6.5 seconds on repeat.
