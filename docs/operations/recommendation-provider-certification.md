...
After both authorities are installed and protected environment values exactly match them, set `OGC_RECOMMENDATION_RUNTIME_ENABLED=true` and `OGC_RECOMMENDATION_OPERATOR_ENABLED=true` only in protected staging. For the delivery drill also provide a decryptable staging-only `RESEND_API_KEY`, `RESEND_WEBHOOK_SECRET`, `RESEND_FROM_EMAIL="Open GEO Console <reports@itheheda.online>"`, `OGC_REPLY_TO_EMAIL="support@itheheda.online"`, and the required `OGC_TEST_EMAIL_RECIPIENT`. Restart Web and deep Worker, verify the catalog opens only for the operator lane, then run a paid Sandbox order through Webhook, deep fulfillment, private HTML/PDF, report-ready email, reply delivery to the support mailbox, and credit settlement. Missing mail values are a blocked acceptance gate; queued messages are not delivery proof.
...
The first live acceptance must also verify the rendered executive and vendor layers against the immutable cells/sources, PDF page count and content, anonymous `404` for HTML/PDF/evidence, one-provider and total-provider failure behavior, rate-limit/timeout handling, refund submission, email Webhook handling, and a real reply arriving at `support@itheheda.online`. A successful legacy website-audit order is not a substitute for this product-specific drill.
