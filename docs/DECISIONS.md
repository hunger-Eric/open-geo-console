...
V4 completion is governed by `config/report-contracts/combined-geo-report-v4.requirements.json` and `npm run report:v4:acceptance`. A task checkbox, design statement, commit message or developer report cannot independently establish completion. Every requirement stays `planned` until actual implementation files, requirement-marked automated tests, successful verification commands and requirement-bound protected-Staging evidence exist. Changing the product meaning requires design approval before changing the registry or tests; the implementation may not weaken acceptance to match its output.
