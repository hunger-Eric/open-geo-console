...
## 2026-07-15: Answer-first V3 may prepare an artifact without activating an evidence-insufficient report

`combined_geo_report_v3` answers the three locked buyer questions before presenting supporting analysis. Each factual sentence must cite eligible direct evidence bound to the same question and one resolved subject; insufficient evidence produces an empty answer card with localized coverage and diagnosis, never model-authored filler. Coverage prose is customer content and therefore passes the persisted report locale gate. Complete target hostnames are preserved through language correction; a narrowly matched legacy translated-TLD placeholder may be restored only from the report's known target URL.

Worker recovery may reuse only terminal, identity-matching public-search ledgers. Source-local timeouts become inaccessible sources, caller/deadline aborts still propagate, interrupted ledgers are replaced, and concurrent waiters ignore completed snapshots whose metadata does not match their requested purpose.

The Worker may materialize canonical HTML and a private PDF before atomic commercial terminalization. If every answer card remains insufficient, the revision stays `ready` but non-active, the job becomes `failed`, the credit is refunded, and refund/email intents are queued. No customer HTML is activated from that revision. This proves only the commercial fail-closed/refund/non-activation boundary; it is a failed V3 product acceptance and is not a deliverable report or permission to weaken evidence thresholds.

Search observations and safely retrieved, question-relevant body text decide whether an answer may exist. Provider identity/qualification decides confidence, source role and diagnosis, never answer existence. A question, notice, article heading or generic role title cannot be a provider identity, and titles never serve as factual evidence. When a candidate-verification page has no accepted provider claim, its body excerpt may still enter Q1 under a traceable domain subject; one source is `limited`, while `verified` still requires two independent registrable domains supporting the same question and subject. Coverage records planned/completed queries, returned observations, retrieval attempts, safe pages and eligible direct evidence as separate facts.

## 2026-07-15: A paid failure may be delivered only through audited non-billable replacement lineage

A terminal paid job is immutable. An explicitly approved replacement binds one order, report, failed job, failed artifact and locked question set; creates no payment, credit reservation or refund request; and activates only a new `replacement` artifact after canonical HTML, private PDF readiness and V3 validation pass. Its terminalizer may complete the replacement while preserving the original order and provider-refund states.

Replacement recovery reuses immutable public-source and answer checkpoints. Snapshot identity includes the exact ordered query plan, unsupported model confidence is downgraded from cited evidence, and a reused historical AI foundation is not revalidated as newly generated prose. Newly generated V3 cards, public-source presentation, localized technical copy and final artifact still pass their prospective gates. Protected staging preview access recognizes a completed replacement; production access remains the one-time secure delivery link.

## 2026-07-15: Exhausted acquisition is deliverable only after execution integrity is preserved

A V3 question with no eligible direct evidence is rendered as one deterministic, nonblank `unresolved` conclusion containing its persisted search-result and retrieval-attempt counts. Three valid unresolved cards are a `completed_limited` courtesy delivery; legacy empty `insufficient` cards remain readable but do not gain this meaning. Internal collection or persistence failure is not evidence absence and cannot directly activate an artifact.

Candidate verification is supplemental to a completed discovery snapshot. If every supplemental query terminates without a successful/partial observation, its snapshot may complete as exhausted; initial question search still requires at least one successful/partial attempt. Long-running snapshot work renews its lease, and a network refresh failure may reuse only an exact completed snapshot with the same authority, query identity, and metadata while marking `refreshFailed=true`. Observation persistence, snapshot materialization, authority mismatch, and missing exact prior snapshots remain fail-closed.

The protected-staging refresh boundary may create a new non-billable V3 evidence revision for the explicitly approved completed replacement report. It never rewrites the replacement, original job, order, credit, or refund. A refresh reuses the previously accepted technical foundation under presentation-only language validation; newly generated answer cards and final HTML still pass current V3 validation and private PDF readiness before atomic activation.
...
V4 completion is governed by `config/report-contracts/combined-geo-report-v4.requirements.json` and `npm run report:v4:acceptance`. A task checkbox, design statement, commit message or developer report cannot independently establish completion. A requirement is `planned` before its real implementation paths, requirement-marked automated tests and verification commands pass; it becomes `implemented` after all three pass; it becomes `verified` only after its requirement-bound protected-Staging evidence also passes. Acceptance succeeds only when every requirement is `verified`. Changing the product meaning requires design approval before changing the registry or tests; the implementation may not weaken acceptance to match its output.

## 2026-07-19: Narrow changes require an explicit, fail-closed scope lock
