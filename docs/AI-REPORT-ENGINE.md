...
7. Prospective `combined_geo_report_v2` jobs retain the public-source forensic graph. `combined_geo_report_v3` uses `generative_search_v1` as its answer mainline: the configured model answers each of exactly three locked questions with native web search and returns same-operation URL citations. Independent retrieval and source auditing may enrich ownership/readiness labels, but may not erase a nonblank ordinary answer or turn missing audit coverage into an empty card. Only a typed provider refusal may produce a refused card; a usable answer with weak or missing citations is rendered as `source_limited`.
...
Each generative V3 card stores the answer/refusal state, normalized same-response sources, answer/source hashes, model/search provenance, and a secondary audit result. The Worker checkpoints all three generated cards before resolving the forensic audit runtime, so an audit outage cannot silently discard already generated answers. The legacy evidence-constrained card shape remains readable for historical artifacts.

## Language and Artifact Readiness

A report's persisted generation locale is immutable. Page analysis, website synthesis and combined-answer prompts all include the shared locale instruction. Generated prose is validated inside each retry boundary; page analysis and website synthesis send only the exact violating field paths to the single corrective model call, then apply one-to-one replacements onto the parsed draft without exposing evidence or permitting missing, duplicate or extra paths. A bounded deterministic cleanup removes residual unapproved Latin fragments from otherwise Chinese replacements while preserving complete target hostnames, source-grounded mixed-Chinese brands and controlled technical terms; an all-English replacement still fails closed. Answer-card coverage reasons are also locale-validated customer prose, and a legacy language-invalid answer checkpoint is regenerated. New combined artifacts run a prospective final prose gate before HTML or internal PDF materialization. Source-original quotes remain verbatim and are excluded from the prose gate; application-owned labels and methodology copy are localized deterministically. Only independently resolved entity names and supported claim subjects can authorize foreign-language proper names.
...
- `report_artifact_revisions`, `combined_report_payloads`, `combined_geo_reports` — versioned V1/V2/V3 HTML payloads and activation lineage; a failed all-insufficient V3 may retain a ready non-active internal artifact, and staging refresh lineages remain protected-staging-only.
