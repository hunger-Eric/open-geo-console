...
A report's persisted generation locale is immutable. **Legacy runtime fact:** current paths may use model correction plus deterministic language-validation behavior, including `report_language_validation_failed` and `repair_wait`; this describes existing behavior only and does not certify the new marker-bearing design. Historical payload parsing does not invoke prospective validation and existing payloads are never rewritten.

**Confirmed target for new marker-bearing Foundation, Free, and Paid paths:** the model is the sole semantic authority for natural-language output. Code retains only non-semantic boundaries: schema, cardinality, IDs, hashes, URLs, evidence ownership, checkpoints, state transitions, persistence, payment, and access. Word lists, regexes, character ratios, substring checks, code language gates, and local semantic fallbacks are not permitted to decide or repair semantic acceptability.
