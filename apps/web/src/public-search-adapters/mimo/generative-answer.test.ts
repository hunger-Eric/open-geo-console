...
describe("MiMo generative answer adapter", () => {
  ...
  it("namespaces citation annotation source ids with the owning question identity so per-question answers do not collide when flattened", async () => {
    const answerFor=(questionId:string)=>{ const response={ok:true,status:200,json:async()=>({id:`resp-${questionId}`,choices:[{message:{content:JSON.stringify({...valid,questionId,sources:[]}),annotations:[{type:"url_citation",url:`https://authority.example/${questionId}/a`,title:"采购指南",summary:"核验交付限制"},{type:"url_citation",url:`https://authority.example/${questionId}/b`,title:"服务说明",summary:null}]}}]})} as Response; return createMiMoGenerativeSearchAnswerProvider({config,fetch:async()=>response}).answerWithSources({...input,questionId}); };
    const answers=await Promise.all(["question-1","question-2","question-3"].map(answerFor));
    const flattened=answers.flatMap((answer)=>answer.sources);
    expect(flattened).toHaveLength(6);
    expect(new Set(flattened.map(({sourceId})=>sourceId)).size).toBe(6);
    for(const answer of answers) for(const [index,source] of answer.sources.entries()) expect(source.sourceId).toBe(`mimo-annotation-${answer.questionId}-${index+1}`);
  });
