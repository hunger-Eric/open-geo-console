...
export function LogAnalysisResults({
  ...
  const detectedBotCount = result.botCoverage.filter((row) => row.status === "detected").length;
  const coverageGroups = useMemo(() => groupCoverageByOperator(result.botCoverage), [result.botCoverage]);
  ...
  return (
    <article className="min-w-0 rounded-md border border-[var(--border)] p-4">
      ...
      <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
        <LogKeyValue label={dictionary.logs.fields.intent} value={dictionary.logs.intentLabels[row.intent]} />
        <LogKeyValue
        ...
        />
        {row.latestDate ? (
          <LogKeyValue label={dictionary.logs.fields.latestDate} value={formatDate(locale, row.latestDate)} />
        ) : null}
        {row.robotsToken ? (
          <LogKeyValue label={dictionary.logs.fields.robotsToken} value={row.robotsToken} mono />
        ) : null}
        {row.docsUrl ? (
          <div>
