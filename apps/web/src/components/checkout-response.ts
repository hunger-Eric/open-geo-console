...
export function getStripeCheckoutRedirect(payload: CheckoutPayload): string | null {
  if (typeof payload.orderId !== "string" || !/^[a-zA-Z0-9_-]{1,128}$/.test(payload.orderId)) return null;
  if (typeof payload.checkoutUrl !== "string") return null;
  try {
