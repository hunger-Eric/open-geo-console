...
export interface ReportV4EvidenceRequirementRegistry {
...
export function verifyReportV4StagingEvidence(
  ...
  const registry = parseEvidenceRegistry(registryValue);
...
function parseEvidenceRegistry(value: unknown): ReportV4EvidenceRequirementRegistry {
  const input = record(value, "report V4 evidence registry");
  if (!Array.isArray(input.requirements) || input.requirements.length === 0) {
    throw new TypeError("report V4 evidence registry requirements must be a nonempty array.");
  }
  const ids = new Set<string>();
  return {
    requirements: input.requirements.map((entry, index) => {
      const requirement = record(entry, `report V4 evidence registry requirements[${index}]`);
      const id = nonblank(requirement.id, `report V4 evidence registry requirements[${index}].id`);
      if (ids.has(id)) throw new TypeError(`Duplicate report V4 evidence registry requirement ID ${id}.`);
      ids.add(id);
      if (!Array.isArray(requirement.verificationCommands) || requirement.verificationCommands.length === 0) {
        throw new TypeError(`${id}.verificationCommands must be a nonempty array.`);
      }
      const commands = requirement.verificationCommands.map((command, commandIndex) => nonblank(
        command,
        `${id}.verificationCommands[${commandIndex}]`
      ));
      if (new Set(commands).size !== commands.length) {
        throw new TypeError(`${id}.verificationCommands must not contain duplicates.`);
      }
      return { id, verificationCommands: commands };
    })
  };
}

function parseRequirementResults(value: unknown, registry: ReportV4EvidenceRequirementRegistry): RequirementResult[] {
