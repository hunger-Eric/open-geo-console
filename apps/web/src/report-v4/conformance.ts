...
} from "./protected-staging-evidence";
...
export type ReportV4ProtectedStagingEvidence = ReportV4StagingVerificationEvidence;
...
export function parseReportV4ProtectedStagingEvidence(
  value: unknown,
  registry: ReportV4RequirementRegistry
): ReportV4ProtectedStagingEvidence {
  return verifyReportV4StagingEvidence(value, registry);
}
  ...
  if (mode === "acceptance") {
    ...
    for (const requirement of registry.requirements) {
      ...
      for (const evidencePath of requirement.runtimeEvidencePaths) {
        ...
        if (!evidence) {
          try {
            evidence = parseReportV4ProtectedStagingEvidence(
