...
/** Splits a propagated sub-budget into the deadline each bounded-concurrency wave receives. */
export function splitPublicSourceSubBudgetMs(totalMs: number, units: number, concurrency = 1): number {
  ...
  if (!Number.isSafeInteger(concurrency) || concurrency < 1) throw new TypeError("Public-source sub-budget concurrency is invalid.");
  return Math.max(1, Math.floor(totalMs / Math.ceil(units / concurrency)));
}
