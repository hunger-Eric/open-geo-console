...
export function resolveCombinedReportContract(environment?: { OGC_COMBINED_REPORT_CONTRACT?: string }): CombinedReportContract {
  ...
  if (value !== "combined_geo_report_v1" && value !== "combined_geo_report_v2" && value !== "combined_geo_report_v3") throw new Error("OGC_COMBINED_REPORT_CONTRACT must select a reviewed combined report contract.");
  return value;
}
