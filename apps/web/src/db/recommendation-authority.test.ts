...
function failedReport(reportId: string, jobId: string, runId: string): RecommendationForensicReportV1 {
  ...
  return {
    ...
    executivePriorities: [1, 2, 3].map((order) => ({ order: order as 1 | 2 | 3, title: `Priority ${order}`, rationale: "Grounded.", evidenceCellIds: [], websiteFindingIds: ["finding-1"] })) as RecommendationForensicReportV1["executivePriorities"],
...
function memoryJob(id: string, reportId: string) { const now = new Date("2030-01-01T00:00:00.000Z"); return { id, reportId, tier: "deep" as const, locale: "en" as const, reason: "standard" as const, stage: "queued" as const, progress: 0, checkpoint: {}, plannedPages: 0, successfulPages: 0, failedPages: 0, attempts: 0, maxAttempts: 3, leaseOwner: null, leaseExpiresAt: null, errorCode: null, publicError: null, creditReservationId: null, createdAt: now, updatedAt: now }; }
