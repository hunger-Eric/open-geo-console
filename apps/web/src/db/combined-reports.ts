...
export async function getAnyActiveCombinedGeoReport(reportId: string): Promise<ActiveCombinedGeoReport | null> {
  return await getActiveCombinedGeoReport(reportId, "combined_geo_report_v4")
    ?? await getActiveCombinedGeoReport(reportId);
}
