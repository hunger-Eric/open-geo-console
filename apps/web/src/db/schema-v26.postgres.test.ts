...
describeDisposablePostgres("schema v26 V4 PostgreSQL constraints", () => {
  ...
  it("keeps snapshots and successful answers immutable and isolates core from enhancement", async () => {
    ...
    try {
      const v26Migrations = DATABASE_MIGRATIONS.slice(0, -databaseMigrationsAfter(26).length);
      await sql.begin(async (tx) => { for (const statement of v26Migrations) await tx.unsafe(statement); });
