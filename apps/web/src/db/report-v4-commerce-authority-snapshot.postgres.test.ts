...
import { loadReportV4CommerceAuthoritySnapshot } from "./report-v4-commerce-authority-snapshot";
...
async function seedHappyPath(sql: postgres.Sql) {
  ...
  const ids = { sessionId: "11111111-1111-4111-8111-111111111111", scenarioId: "22222222-2222-4222-8222-222222222222",
  ...
  const configHash = "a".repeat(64), config = `v4-config-${configHash}`;
  ...
  await sql`INSERT INTO payment_orders(id,checkout_idempotency_hmac,provider,provider_checkout_id,provider_payment_id,report_id,site_snapshot_id,site_key,
    ...
    VALUES(${ids.order},'SECRET_KEY','airwallex','checkout-provider','payment-provider',${ids.report},${ids.snapshot},'SECRET_URL','SECRET_EMAIL','SECRET_EMAIL','v1',
  ...
  await sql`INSERT INTO access_keys(id,key_prefix,key_hmac,payment_order_id,status,credits_remaining,expires_at) VALUES(${ids.access},'SECRET_KEY','SECRET_KEY',${ids.order},'exhausted',0,now()+interval '1 day')`;
  await sql`INSERT INTO credit_ledger(id,access_key_id,report_id,idempotency_key,payment_order_id,credits,status,reserved_at,settled_at) VALUES(${ids.credit},${ids.access},${ids.report},'SECRET_KEY',${ids.order},1,'settled',now(),now())`;
