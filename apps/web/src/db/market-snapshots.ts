...
function parseSource(input: SourceEvidenceInput): Omit<MarketSourceEvidenceRow, "createdAt"> { if (!["available","inaccessible","not_retrieved"].includes(input.retrievalState)) throw new TypeError("Unsupported source retrieval state."); if (!["company_owned","earned_editorial","directory_or_reference","community_or_ugc","institution","social","unknown"].includes(input.sourceCategory)) throw new TypeError("Unsupported source category."); const excerpt = input.excerpt == null ? null : privateSafeText(input.excerpt, "excerpt", 1_200); const excerptHash = input.excerptHash == null ? null : hashText(input.excerptHash, "excerptHash"), contentHash = input.contentHash == null ? null : hashText(input.contentHash, "contentHash"); if (input.retrievalState === "available" ? (!excerpt || !excerptHash || !contentHash) : (excerpt || excerptHash || contentHash)) throw new TypeError("Source retrieval state and retained content are inconsistent."); return { id: bounded(input.id, "source.id", 256), snapshotId: bounded(input.snapshotId, "snapshotId", 256), observationId: bounded(input.observationId, "observationId", 256), canonicalUrl: publicUrl(input.canonicalUrl), registrableDomain: bounded(input.registrableDomain, "registrableDomain", 255), retrievalState: input.retrievalState, excerpt, excerptHash, contentHash, sourceCategory: input.sourceCategory, entities: validatePublicMetadata(input.entities ?? [], "entities"), claims: validatePublicMetadata(input.claims ?? [], "claims"), contradictions: validatePublicMetadata(input.contradictions ?? [], "contradictions"), evidenceFamilyIdentity: bounded(input.evidenceFamilyIdentity, "evidenceFamilyIdentity", 256), retrievedAt: validDate(input.retrievedAt, "retrievedAt"), expiresAt: validDate(input.expiresAt, "expiresAt") }; }
function validatePublicMetadata(value: unknown, label: string, depth = 0): unknown { if (depth > 4 || Buffer.byteLength(JSON.stringify(value), "utf8") > 8_192) throw new TypeError(`${label} exceeds public metadata bounds.`); if (value === null || typeof value === "boolean" || (typeof value === "number" && Number.isFinite(value))) return value; if (typeof value === "string") { if (value.length > 2_048) throw new TypeError(`${label} string is too long.`); assertNoPrivateIdentity(value); return value; } if (Array.isArray(value)) return value.map((item, index) => validatePublicMetadata(item, `${label}[${index}]`, depth + 1)); if (value && typeof value === "object") { const output: Record<string, unknown> = {}; for (const [key, item] of Object.entries(value)) { const normalized = key.toLowerCase().replace(/[^a-z0-9]/g, ""); if (key.length > 64 || !PUBLIC_METADATA_KEYS.has(normalized)) throw new TypeError(`${label} contains unsupported key: ${key}`); output[key] = validatePublicMetadata(item, `${label}.${key}`, depth + 1); } return output; } throw new TypeError(`${label} contains unsupported metadata.`); }
function assertNoPrivateIdentity(value: string): void { if (/\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b/i.test(value) || /\b(?:report|job|order|customer|token)[ _-]*(?:id|identity|identifier)\b/i.test(value) || /^(?:report|job|order|customer|token)[_-][a-z0-9-]{4,}$/i.test(value.trim()) || /authorization\s*[:=]|\bbearer\s+\S+|\b(?:api[-_ ]?key|access[-_ ]?token|client[-_ ]?secret|secret)\b\s*[:=]?\s*\S*/i.test(value) || /submitted[ _-]*url|client[ _-]*ip/i.test(value) || /\b(?:\d{1,3}\.){3}\d{1,3}\b/.test(value) || /\b(?:[a-f0-9]{1,4}:){2,7}[a-f0-9]{1,4}\b/i.test(value)) throw new TypeError("Shared public-search data contains private or sensitive identity material."); }
