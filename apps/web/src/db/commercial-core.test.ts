...
describe("commercial persistence pure contracts", () => {
  ...
  it("installs locale, correction, reason, and checkpoint persistence migrations", () => {
    ...
    expect(migration).toContain("CHECK (reason IN ('standard','system_recovery','locale_correction','staging_regeneration','paid_report_correction','staging_artifact_refresh'))");
