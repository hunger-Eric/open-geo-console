...
import { DATABASE_MIGRATIONS, V21_DATABASE_MIGRATIONS } from "./migrations";
...
describeDisposablePostgres("schema v21 prospective V3 artifact scope", () => {
  ...
  it("preserves V1/V2 rows and admits only the reviewed V3 scope", async () => {
    ...
    try {
      const v20 = DATABASE_MIGRATIONS.slice(0, -V21_DATABASE_MIGRATIONS.length);
      ...
      expect(DATABASE_MIGRATIONS.slice(-V21_DATABASE_MIGRATIONS.length)).toEqual(V21_DATABASE_MIGRATIONS);
