  ...
  beforeAll(async()=>{
    ...
    for(const row of records){
      ...
      await sql`INSERT INTO report_business_question_sets(id,report_id,order_id,revision,locale,region,status,confidence,generation_rule_version,neutralization_version,profile_evidence_identity,content_hash,neutral_content_hash,payload,confirmed_at,locked_at)
        VALUES(${row.questionSetId},${row.reportId},${row.orderId},1,'zh-CN','CN','candidate','high','v1','identity-neutral-v1','profile',null,null,null,null,null)`;
      for (const [ordinal, purpose] of [[1,"core_service_discovery"],[2,"customer_region_fit"],[3,"purchase_delivery_risk"]] as const) {
        await sql`INSERT INTO report_business_questions(id,question_set_id,ordinal,purpose,generated_text,private_text,neutral_public_text,neutral_content_hash)
          VALUES(${`question-${row.outcome}-${ordinal}-${suffix}`},${row.questionSetId},${ordinal},${purpose},${`问题 ${ordinal}`},${`问题 ${ordinal}`},${`公开问题 ${ordinal}`},${`question-hash-${row.outcome}-${ordinal}`})`;
      }
