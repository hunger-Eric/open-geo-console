...
export async function createProductionPublicSourceForensicsDependencies(environment:NodeJS.ProcessEnv=process.env):Promise<PublicSourceForensicsDependencies|null>{
  try { await resolveProductionPublicSearchRuntime({environment,getAuthority:getActivePublicSearchSurfaceAuthority}); } catch { return null; }
  return null;
}
function required(value:string|undefined,name:string){if(!value?.trim())throw new Error(`${name} is required.`);return value.trim();}
function assertRow(row:PublicSearchSurfaceAuthorityRow,identity:PublicSearchAdapterIdentity,environment:"staging"|"production",locale:string,region:string){if(!row.active||row.environment!==environment||row.adapterId!==identity.adapterId||row.providerId!==identity.providerId||row.productId!==identity.productId||row.modelId!==identity.modelId||row.adapterVersion!==identity.adapterVersion||row.surfaceId!==identity.surface.surfaceId||row.surfaceVersion!==identity.surface.surfaceVersion||!row.localeCapabilities.includes(locale)||!row.regionCapabilities.includes(region))throw new Error("Public-search authority identity mismatch.");}
