...
export async function GET(request: Request, context: RouteContext) {
  ...
  const response = new NextResponse(`<!doctype html><html lang="${report.reportLocale}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${zh ? "打开报告" : "Open report"}</title></head><body style="font-family:system-ui;background:#f5f5f0;color:#17202a;padding:32px"><main style="max-width:560px;margin:12vh auto;background:white;padding:32px;border-radius:14px"><p style="color:#64748b;font-size:12px">OPEN GEO CONSOLE</p><h1>${zh ? "确认打开诊断报告" : "Confirm report access"}</h1><p>${zh ? "点击下方按钮后，本设备将获得报告访问权限。链接在报告访问有效期内可重复使用。" : "After you continue, this device will receive report access. The link stays reusable until the report access expires."}</p><form method="post" action="${escapeHtml(action)}"><input type="hidden" name="token" value="${escapeHtml(token)}"><button type="submit" style="border:0;background:#111827;color:white;padding:12px 18px;border-radius:8px;cursor:pointer">${zh ? "安全打开报告" : "Open report securely"}</button></form></main></body></html>`, {
    status: 200,
...
export async function POST(request: Request, context: RouteContext) {
  ...
  const destination = verified.artifactScope === "recommendation_forensics_v1" || verified.artifactScope === "combined_geo_report_v1" || verified.artifactScope === "combined_geo_report_v2" || verified.artifactScope === "combined_geo_report_v3" || verified.artifactScope === "combined_geo_report_v4"
