...
function isRetryableContractError(error: unknown): boolean { return error instanceof TypeError || (error instanceof AiClientError && error.status !== 401 && error.status !== 403 && (error.code === "network" || error.code === "timeout" || /invalid json|non-json|response envelope/i.test(error.message))); }
